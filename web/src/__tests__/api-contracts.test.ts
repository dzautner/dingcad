...
describe("Auth endpoints", () => {
  it("login: POST /auth/login with email and password", async () => {
    const body = { token: "jwt", token_expires_at: 9999999999, user: { id: "1", email: "a@b.fi" } };
  ...
  it("register: POST /auth/register with email, password, name", async () => {
    const body = { token: "jwt", token_expires_at: 9999999999, user: { id: "1" } };
    vi.mocked(fetch).mockResolvedValueOnce(mockResponse(body));
...
describe("Error handling", () => {
  ...
  it("on 401 with successful refresh, retries the original request", async () => {
    ...
    // Refresh succeeds
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse({ token: "new-token", token_expires_at: 9999999999 })
    );
    // Retry succeeds
    vi.mocked(fetch).mockResolvedValueOnce(
      mockResponse([{ id: "p1" }])
...
describe("Token management", () => {
  ...
  it("setToken persists to localStorage", () => {
    ...
    expect(localStorage.getItem("helscoop_token")).toBe("persist-token");
  });
  ...
  it("setToken with expiresAt stores expiry in localStorage", () => {
    setToken("token", 1234567890);
    expect(localStorage.getItem("helscoop_token_expires_at")).toBe("1234567890");
  });
});
...
describe("PDF and CSV export", () => {
  it("exportPdf: GET /projects/:id/pdf?lang=<lang> with auth header", async () => {
    ...
    expect((opts.headers as Record<string, string>).Authorization).toBe("Bearer test-jwt-token-abc");
    expect(mockLink.download).toBe("helscoop_Test_Project.pdf");
  });
  ...
  it("exportBOMCsv: GET /bom/export/:projectId?format=csv with auth header", async () => {
    ...
    expect((opts.headers as Record<string, string>).Authorization).toBe("Bearer test-jwt-token-abc");
    expect(mockLink.download).toBe("helscoop_My_Project.csv");
  });
